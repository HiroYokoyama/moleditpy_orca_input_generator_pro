...
class TestMergeAllowlists(unittest.TestCase):
    ...
    def test_context_sets_are_unioned(self):
        ...
        b = {"context": {"add_export_action"}}
        result = _merge_allowlists(a, b)
        self.assertIn("get_main_window", result["context"])
        self.assertIn("add_export_action", result["context"])
...
class TestLoadSiteAllowlist(unittest.TestCase):
    ...
    def test_context_list_form(self):
        with tempfile.TemporaryDirectory() as d:
            self._write(Path(d), {"context": ["add_export_action"]})
            result = _load_site_allowlist(Path(d))
        self.assertIn("add_export_action", result.get("context", set()))
...
class TestPluginFileChecker(unittest.TestCase):
    ...
    def test_qt_inherited_set_window_title_not_flagged(self):
        self.assertEqual(self._issues("def f(mw):\n    mw.setWindowTitle('x')\n"), [])
    ...
    def test_add_export_action_is_known_context_attr(self):
        source = "def initialize(context):\n    context.add_export_action('x', lambda: None)\n"
        self.assertEqual(self._issues(source, check_context=True), [])

    def test_mark_project_modified_is_known_context_attr(self):
    ...
    def test_dedup_same_access_not_doubled_on_same_line(self):
        source = "def f(mw):\n    mw.nonexistent()\n    mw.nonexistent()\n"
        issues = [i for i in self._issues(source) if i.code == "UNKNOWN_MW_ATTR"]
        # Two distinct lines may produce two issues; no line should be triple-counted
