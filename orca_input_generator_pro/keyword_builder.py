...
class OrcaKeywordBuilderDialog(Dialog3DPickingMixin, QDialog):
    ...
    def _capture_constraints(self):
        ...
        for r in range(self.constraint_table.rowCount()):
            ...
            is_scan = (
                chk_widget.findChild(QCheckBox).isChecked() if chk_widget else False
            )
    ...
    def get_inferred_category(self, text):
        ...
        if text_upper in [
            ...
            m.upper() for m in ["CASSCF", "NEVPT2", "DLPNO-NEVPT2", "MRCI"]
        ]:
    ...
    def setup_constraints_tab(self):
        ...
        info_label = QLabel(
            "Select 1-4 atoms in the 3D view to add constraints or scans.\n"
        ...
        self.constraint_table.setHorizontalHeaderLabels(
            ["Type", "Indices", "Value", "Scan?", "Start", "End", "Steps"]
        )
    ...
    def get_constraints_text(self):
        ...
        for r in range(self.constraint_table.rowCount()):
            ...
            if not c_type_item or not idx_item:
                continue
            ...
            chk_widget = self.constraint_table.cellWidget(r, 3)
            if not chk_widget:
                continue  # Should have QCheckBox inside WIdget
            ...
            if is_scan:
                ...
                if not start_item or not end_item or not steps_item:
                    continue
    ...
    def update_preview(self):
        ...
        elif "OptH" in job_txt:
            route_parts.append("OptH")
            if opt_conv:
                route_parts.append(opt_conv)
        elif "Only (Opt)" in job_txt:
            route_parts.append(opt_conv if opt_conv else "Opt")
        elif "OptTS" in job_txt:
            route_parts.append("OptTS")
            if opt_conv:
                route_parts.append(opt_conv)
        elif "Only (Freq)" in job_txt:
            route_parts.append("Freq")
        ...
        if self.scf_sloppy.isChecked():
            route_parts.append("SloppySCF")
        elif self.scf_loose.isChecked():
            route_parts.append("LooseSCF")
        elif self.scf_normal.isChecked():
            route_parts.append("NormalSCF")
        elif self.scf_strong.isChecked():
            route_parts.append("StrongSCF")
        elif self.scf_tight.isChecked():
            route_parts.append("TightSCF")
        elif self.scf_verytight.isChecked():
            route_parts.append("VeryTightSCF")
    ...
    def get_extra_blocks_text(self):
        ...
        # 1. TD-DFT
        if (
            getattr(self, "tddft_enable", None) is not None
            and self.tddft_enable.isChecked()
        ):
            block = (
                f"%tddft\n"
    ...
    def parse_route(self, route):
        ...
        for t in tokens:
            ...
            if tu == "SLOPPYSCF":
                ...
                self.scf_loose.setChecked(True)
            elif tu == "NORMALSCF":
                self.scf_normal.setChecked(True)
            elif tu == "STRONGSCF":
                self.scf_strong.setChecked(True)
            elif tu == "TIGHTSCF":
                self.scf_tight.setChecked(True)
            elif tu == "VERYTIGHTSCF":
        ...
        for section in block_sections:
            ...
            if bname == "tddft":
                ...
                if nr:
                    self.tddft_nroots.setValue(int(nr.group(1)))
                ...
                const_match = re.search(
                    r"Constraints\s+(.*?)\s+end", bcontent, re.I | re.S
                )
                if const_match:
    ...
    def _add_parsed_constraint(
        self, c_type, indices_str, value, is_scan, end="0.0", steps="10"
    ):
        row = self.constraint_table.rowCount()
        self.constraint_table.insertRow(row)
